...
public class UserServiceImpl implements UserService {
    ...
    public Boolean isUserExist(Long userId) {
        log.info("Поступил запрос на уточнение существования пользователя с id: {}", userId);
        return userRepository.existsById(userId);
    }

    @Override
    public UserShortDto getUserShortInfo(Long userId) {
        log.info("Поступил запрос на краткую информацию о пользователе с id: {}", userId);
    ...
    public List<UserShortDto> getShortInfoOfUsers(List<Long> usersIds) {
        log.info("Поступил запрос на краткую информацию о пользователях по списку id: {}", usersIds);
